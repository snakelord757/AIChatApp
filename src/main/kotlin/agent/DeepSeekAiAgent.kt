...
class DeepSeekAiAgent(
    ...
    private fun summaryMessages(history: List<ChatMessage>): List<ChatMessage> {
        val prompt = """
            Составь краткое, но полное summary диалога для дальнейшего использования как контекста.
            Summary должно сохранять важные факты, цели пользователя, принятые решения, текущие настройки,
            незавершенные задачи и технические детали, необходимые для продолжения.
            Не добавляй лишних рассуждений. Верни только текст summary.
        """.trimIndent()
