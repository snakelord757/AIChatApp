...
class PromptedStageAgentTest {
    ...
    fun `planning agent adapts broad prompts to invariants before failing`() {
        ...
        val systemPrompt = client.messages.first().content
        assert(systemPrompt.contains("first try to create a compliant plan"))
        assert(systemPrompt.contains("do not fail only because some requested options are disallowed"))
