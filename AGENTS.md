...
- Контекст обычного чата собирается по явному `modelContextWindowTokens`: лимит передаётся провайдеру и применяется локально; приложение сохраняет фактические `prompt_tokens` для чата/ветки и перед следующим сообщением тихо сжимает контекст в sticky facts при достижении 95% окна.
...
- Актуальный хэш: 4a3cb9d36e99a26db456bb944474e21511d4eab5
- Саммари:
...
### Replace context strategies with token-window sticky facts

- Дата выполнения: 2026-07-09
- Актуальный хэш: 95ef0b73e9c5ec11478323e48a8abfd222672129
- Саммари:
  - Удалены старые стратегии контекста и связанные настройки; размер контекстного окна теперь задаётся только явным лимитом `MODEL_CONTEXT_WINDOW_TOKENS`.
  - При достижении token budget агент без уведомления пользователя извлекает sticky facts и использует их для дальнейшего контекста; ручной просмотр сохранён через `/facts`.
  - Обновлены настройки CLI, конфигурация, документация и focused unit tests; версия приложения поднята до `1.18.1`.
  - Amend: исправлен оставшийся вызов удалённой window-сигнатуры в источнике personal memory и актуализировано ожидание контекстного теста; все 233 теста проходят, версия поднята до `1.18.2`.
  - Amend: `MODEL_CONTEXT_WINDOW_TOKENS` теперь передаётся провайдеру как `options.num_ctx` в обычном агенте и stage-agent, локальная сборка контекста использует тот же лимит; версия поднята до `1.18.3`.

### Track sticky facts context occupancy

- Дата выполнения: 2026-07-09
- Актуальный хэш: c1fda93
- Саммари:
  - Приложение сохраняет фактические `prompt_tokens` последнего основного ответа модели для основного чата, веток и checkpoint, а `MODEL_CONTEXT_WINDOW_TOKENS` остаётся единым лимитом для провайдера и локальной сборки контекста.
  - Перед следующим запросом локально оцениваются добавленные сообщения; на 95% окна выполняется тихое извлечение sticky facts, после чего отправляется заново собранный контекст с фактами и историей в заданном лимите.
  - Добавлены persistence/regression tests, обновлены CLI/README; версия приложения поднята до `1.19.0`.
...
### Generalize model provider support

- Дата выполнения: 2026-07-06
- Актуальный хэш: 2afae588787fe4b0e0b5f42a4bf021436f9eec7a
- Саммари:
  - Приложение отвязано от DeepSeek-специфичных настроек: добавлены нейтральные `MODEL_*` параметры, OpenAI-compatible provider flow и fallback старых `DEEPSEEK_*` ключей.
  - Добавлена команда `/models`, которая при каждом вызове загружает доступные модели через `GET /models`, и выбор модели через `/settings` без ввода API-ключей.
  - Stage-запросы получили строгий `response_format=json_schema`, локальную валидацию JSON-контракта и повтор при нарушении формата.
  - Planning stage дополнительно защищен от прямых финальных ответов: успешный `output` должен быть планом с маркером `Plan:` и не должен содержать first-person ответ вместо плана.
